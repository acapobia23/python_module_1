...
It can find files in the same folder or inside ex0, ex1, ..., ex7.
"""
...
def _load_module_from_exercise_file(exercise_file_name):
    """Load an exercise module from the current folder or ex* subfolders."""
    base_dir = Path(__file__).resolve().parent
    direct_file = base_dir / f"{exercise_file_name}.py"

    if direct_file.exists():
        target_file = direct_file
    else:
        matches = sorted(base_dir.glob(f"ex*/{exercise_file_name}.py"))
        if not matches:
            raise ImportError(f"Could not find {exercise_file_name}.py")
        target_file = matches[0]

    spec = spec_from_file_location(exercise_file_name, target_file)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not import module from {target_file}")
...
def test_ft_exercise(exercise_file_name):
    ...
    try:
        # Import your exercise file from current folder or ex* subfolders.
        ft_module = _load_module_from_exercise_file(exercise_file_name)
        ...
        print(
            """   Make sure your file exists in the same
            folder as main.py or inside ex0/ex1/... subfolders"""
        )
